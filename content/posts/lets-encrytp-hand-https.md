...
description: >-
  这篇文章主要讲的就是如何让自己的网站免费从HTTP升级为HTTPS，使用的是 Let's Encrypt的证书。实际上也就是一个Let's Encrypt
  免费证书获取教程 。 为什么要上HTTPS，说一个小故事。  从前有个网站，后来有个广告，没了
  作为一个博客，自然不想让在被别人浏览的时候，出现莫名的广告（运营商劫持）。这时候，HTTPS的价值就体现出来了。
  HTTPS的好处还有很多，但单单这一条，就够我花心思去升级了。 （其实主要目的是提升网站Biger :）  Let's Encrypt 简介 
  如果要启用HTTPS，我们就需要从证书授权机构(以下简称CA) 处获取一个证书，Let's
tags:
  - https教程
  - certbot教程
  - 免费ssl证书
